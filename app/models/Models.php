...
class BlogArticle extends BaseModel
{
    ...
    public static function published(int $limit = 0, int $offset = 0): array
    {
        if ($limit > 0) {
            return Database::fetchAll(
                'SELECT * FROM blog_articles WHERE status = ? ORDER BY published_at DESC LIMIT ? OFFSET ?',
                ['published', $limit, $offset]
            );
        }
        return Database::fetchAll(
            'SELECT * FROM blog_articles WHERE status = ? ORDER BY published_at DESC',
            ['published']
        );
    }
    ...
    {
        return (int)(Database::fetchOne(
            'SELECT COUNT(*) as c FROM blog_articles WHERE status = ?', ['published']
        )['c'] ?? 0);
    }
    ...
    public static function related(int $excludeId, string $category = '', int $limit = 3): array
    {
        if ($category !== '') {
            $rows = Database::fetchAll(
                'SELECT * FROM blog_articles WHERE status = ? AND id != ? AND category = ? ORDER BY published_at DESC LIMIT ?',
                ['published', $excludeId, $category, $limit]
            );
            if (count($rows) >= $limit) return $rows;
        }
        return Database::fetchAll(
            'SELECT * FROM blog_articles WHERE status = ? AND id != ? ORDER BY published_at DESC LIMIT ?',
            ['published', $excludeId, $limit]
        );
    }
    ...
    public static function uniqueSlug(string $base, int $excludeId = 0): string
    {
        ...
        $count = 1;
        while (true) {
            $row = Database::fetchOne('SELECT id FROM blog_articles WHERE slug = ?', [$slug]);
            if (!$row || (int)$row['id'] === $excludeId) break;
            $slug = $base . '-' . $count++;
        }
        return $slug;
    }
