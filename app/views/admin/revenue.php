...
<div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
  <table class="w-full text-sm">
    <thead><tr class="text-xs text-gray-400 bg-gray-50 border-b border-gray-100">
      ...
      <th class="px-5 py-3 text-right font-medium">Invoice</th>
    </tr></thead>
    <tbody class="divide-y divide-gray-50">
      <?php foreach ($payments as $p): ?>
      <tr class="hover:bg-gray-50/50">
        <td class="px-5 py-3 text-gray-500 text-xs"><?= date('M j, Y', strtotime($p['created_at'])) ?></td>
        <td class="px-5 py-3">
          <div class="text-gray-900"><?= Helpers::e($p['user_name']) ?></div>
          <div class="text-xs text-gray-400"><?= Helpers::e($p['user_email']) ?></div>
        </td>
        <td class="px-5 py-3 text-gray-600"><?= Helpers::e($p['description'] ?? 'Subscription') ?></td>
        <td class="px-5 py-3 text-right font-semibold text-gray-900">$<?= number_format($p['amount'],2) ?> <?= $p['currency'] ?></td>
